...
export function Wardrobe({ initialProducts }: WardrobeProps) {
  ...
  const handleGenerateOutfit = useCallback(async () => {
    if (isGeneratingOutfit) return
    if (!portraitPreviewUrl) {
      setOutfitError('Upload a portrait before generating an outfit.')
      return
    }
    if (!hasAnyEquipped) {
      setOutfitError('Equip at least one slot using your saved finds first.')
      return
    }
  ...
  const renderEquipmentSlotButton = (
    ...
    return (
      <Popover
        key={slotId}
        open={isSelected}
        onOpenChange={(open: boolean) => setSelectedSlot(open ? slotId : null)}>
        ...
        <PopoverContent
          ...
          <div className="grid gap-3">
            ...
            {slotOptions.length > 0 ? (
                    ...
                    <button
                      ...
                      {option.imageUrl ? (
                        <button
                          ...
                          className="relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-2xl border border-white/50 bg-white/40 transition hover:opacity-90 dark:border-white/10 dark:bg-white/10"
                        >
                          <Image
                            src={option.imageUrl}
                            alt={option.name}
              ...
              <p className="text-xs text-muted-foreground">
                No saved items match this slot yet. Swipe relevant gear in Step
                1 to add options.
              </p>
            )}
            {isEquipped ? (
      ...
      <div className="mx-auto w-full max-w-6xl space-y-6">
        ...
        {activeStep === 'discover' ? (
          ...
          <section className="flex min-h-[calc(100vh-12rem)] max-h-[calc(100vh-12rem)] flex-col overflow-hidden rounded-3xl border border-dashed border-white/30 bg-white/5 pt-6 backdrop-blur-2xl transition-colors duration-300 dark:bg-slate-900/70">
            <header className="flex flex-wrap items-start justify-between gap-4 border-b border-white/30 px-6 pb-5"></header>
                        ...
                        <div
                          ...
                          {isPortraitUploading ? (
                            ...
                            <Button
                              ...
                              className="relative h-full w-full cursor-pointer bg-white">
                              <Image
                            ...
                            <Button
                              ...
                              className="flex h-full w-full cursor-pointer items-center justify-center rounded-[28px] bg-white p-2 text-xs text-gray-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 dark:text-gray-600"
                        ...
                        <div className="flex flex-col items-center gap-2 text-center">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button
                              ...
                              disabled={isAgentRunning || isPortraitUploading}
                              className="cursor-pointer text-xs">
                          ...
                          {outfitError ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              Outfit updated—save or regenerate anytime.
                            </p>
                          ) : null}
                        </div>
                      </div>
